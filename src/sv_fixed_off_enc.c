...
/*									    */
/*									    */
/* MODULE NAME : sv_fixed_off_enc.c					    */
/*									    */
/* MODULE DESCRIPTION :							    */
/*	Fixded offset SV encoder (9-2+90-5 extensions)			    */
/*									    */
/* MODIFICATION LOG :							    */
/*  Date     Who			Comments			    */
/* --------  ---  ------   -------------------------------------------	    */
/* 08/11/12  HSF	   Modified header to include Apache License	    */
/* 08/15/12  HSF	   Changed encoding of Confrev from 5->4 bytes	    */
/*			   Chaged smpcnt from 3->2 bytes, both due to	    */
/*			   fixed length encoding change from integer	    */
/*			   to octetstring.				    */
/*			   Fixed memory overwrite due to memory		    */
/*			   cacluation issue regarding UTC_TIMESTAMP	    */
...
#define SIZE_OF_SMPCNT 4			//changed from 5->4 8/15
//  smpcnt					82 02 xx xx 			4
#define SIZE_OF_CONFREV	 6
// confrev					83 04 xx xx xx xx 		6
...
ASDU_ENC_INFO *pMyCurrentASDUInfo;
	...
	if (sv_optflds & SV_INCLUDE_UTC_TIME)	//meed to account for UTC_TIMESTAMP, added 8/11
	  asdu_size+=10;

	if( smpMod!=NULL)	//added 8/11 to allow SMPMOD to be sent per implementation agreements
...
#define SMPCNT_TAG 0x82
	  ...
	  *pEncBuffer++ = 0x02;			//changed from 3->2 8/11 integer->octetstromg
	  ...
	  pEncBuffer +=1;		      //changed from 2->1 8/11 integer->octetstromg
...
#define CONFREV_TAG 0x83
	  ...
	  *pEncBuffer++ = 0x4;		  //length of the confrev making sure the value is always positive
					  //changed from 5->4 due to integer->octestring 8/11
	  ++pEncBuffer;
	  *(ST_UINT32 *)pEncBuffer = htonl(confrev);	/* SPDU Version	*/
	   pEncBuffer += 3;		 //changed from 4->3 8/11 integer->octetstromg
